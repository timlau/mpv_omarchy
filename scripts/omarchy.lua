...
local user_opts = {
  ...
  -- Colors and style (Tokyo Night)
  osc_color = "#000000",                  -- accent color of the OSC and title bar
  window_title_color = "#a9b1d6",         -- color of the title in borderless/fullscreen mode
  window_controls_color = "#a9b1d6",      -- color of the window controls (close, minimize, maximize) in borderless/fullscreen mode
  windowcontrols_close_hover = "#7aa2f7", -- color of close window control on hover
  windowcontrols_max_hover = "#7aa2f7",   -- color of maximize window controls on hover
  windowcontrols_min_hover = "#7aa2f7",   -- color of minimize window controls on hover
  title_color = "#a9b1d6",                -- color of the title (above seekbar)
  cache_info_color = "#a9b1d6",           -- color of the cache information
  seekbarfg_color = "#7aa2f7",            -- color of the seekbar progress and handle
  seekbarbg_color = "#a9b1d6",            -- color of the remaining seekbar
  seekbar_cache_color = "#c0caf5",        -- color of the cache ranges on the seekbar
  volumebar_match_seek_color = false,     -- match volume bar color with seekbar color (ignores side_buttons_color)
  time_color = "#a9b1d6",                 -- color of the timestamps (below seekbar)
  chapter_title_color = "#a9b1d6",        -- color of the chapter title (above seekbar)
  side_buttons_color = "#a9b1d6",         -- color of the side buttons (audio, subtitles, playlist, etc.)
  middle_buttons_color = "#a9b1d6",       -- color of the middle buttons (skip, jump, chapter, etc.)
  playpause_color = "#a9b1d6",            -- color of the play/pause button
  held_element_color = "#c0caf5",         -- color of the element when held down (pressed)
  hover_effect_color = "#7aa2f7",         -- color of a hovered button when hover_effect includes "color"
  thumbnail_border_color = "#a9b1d6",     -- color of the border for thumbnails (with thumbfast)
  thumbnail_border_outline = "#7aa2f7",   -- color of the border outline for thumbnails
  ...
  title_mbtn_right_command = "script-binding select/select-watch-history; script-message-to omarchy osc-hide",

  -- playlist button mouse actions
  playlist_mbtn_left_command = "script-binding select/menu; script-message-to omarchy osc-hide",
  playlist_mbtn_right_command = "script-binding select/select-playlist; script-message-to omarchy osc-hide",

  -- volume mouse actions
  vol_ctrl_mbtn_left_command = "no-osd cycle mute",
  vol_ctrl_mbtn_right_command = "script-binding select/select-audio-device; script-message-to omarchy osc-hide",
  vol_ctrl_wheel_down_command = "no-osd add volume -5",
  vol_ctrl_wheel_up_command = "no-osd add volume 5",

  -- audio button mouse actions
  audio_track_mbtn_left_command = "script-binding select/select-aid; script-message-to omarchy osc-hide",
  ...
  sub_track_mbtn_left_command = "script-binding select/select-sid; script-message-to omarchy osc-hide",
  ...
  chapter_prev_mbtn_right_command = "script-binding select/select-chapter; script-message-to omarchy osc-hide",

  chapter_next_mbtn_left_command = "add chapter 1",
  chapter_next_mbtn_mid_command = "show-text ${chapter-list} 3000",
  chapter_next_mbtn_right_command = "script-binding select/select-chapter; script-message-to omarchy osc-hide",

  -- chapter title (below seekbar) mouse actions
  chapter_title_mbtn_left_command = "script-binding select/select-chapter; script-message-to omarchy osc-hide",
  ...
  playlist_prev_mbtn_right_command = "script-binding select/select-playlist; script-message-to omarchy osc-hide",

  playlist_next_mbtn_left_command = "playlist-next",
  playlist_next_mbtn_mid_command = "show-text ${playlist} 3000",
  playlist_next_mbtn_right_command = "script-binding select/select-playlist; script-message-to omarchy osc-hide",
...
local function sanitize_title(raw_title, should_escape_ass)
  ...
  title = title:gsub("\n", " ")
  if title ~= "" then
    if should_escape_ass then
      return mp.command_native({ "escape-ass", title })
    end
    return title
  end
  return "mpv"
...
local function setup_slider_value_tracking(element)
  element.eventresponder["mouse_move"] = function(elem)
    ...
    if elem.state.lastseek == nil or elem.state.lastseek ~= setvol then
      mp.commandv("osd-msg", "set", "volume", setvol)
      elem.state.lastseek = setvol
    end
  end
  element.eventresponder["reset"] = function(elem)
    elem.state.lastseek = nil
  end
end
...
local function osc_init()
  ...
  -- Helper function to calculate playlist button visibility (defined here where state and osc_param are available)
  local function is_playlist_button_visible(nojump, noskip)
    local min_width = state.is_image and MIN_SMALL_SCREEN_WIDTH or MIN_NORMAL_SCREEN_WIDTH
    local adjustment = nojump + noskip * (nojump == 0 and 1 or 10)
    return osc_param.playresx >= (min_width - adjustment)
  end
  ...
  local function create_button_with_softrepeat(name, content_val, should_softrepeat)
    local ne = new_element(name, "button")
    ne.content = content_val
    ne.softrepeat = should_softrepeat == true
    return ne
  end
  ...
  ne.tooltipF = function()
    ...
    local formatted_volume = volume % 1 == 0 and string.format("%.0f", volume) or string.format("%.1f", volume)
  ...
  -- Use the helper function to setup common slider value tracking
...
local function process_event_mouse_move(action)
  ...
  then
    if user_opts.bottomhover then -- if enabled, only show osc if mouse is hovering at the bottom of the screen (where the UI elements are)
      ...
      if
          mouseY > osc_param.playresy - (user_opts.bottomhover_zone or DEFAULT_BOTTOM_HOVER_ZONE)
          or ((user_opts.window_top_bar == "yes" or not (state.border and state.title_bar)) or state.fullscreen)
          and (mouseY < MOUSE_HOVER_TOP_THRESHOLD and top_hover)
      then
        show_osc()
      else
        state.touchtime = nil
...
local function process_event(source, what)
  ...
  elseif what == "up" then
    process_event_up(action)
  elseif source == "mouse_move" then
    process_event_mouse_move(action)
  end
